...
export async function POST(req, { params }) {
  try {
    ...
    const foodItem = await prisma.foodItem.findUnique({ where: { id: Number.parseInt(id) } })
    ...
    return NextResponse.json(
      { error: "Failed to associate tag with food item", details: error.message },
      { status: 500 },
    )
  }
}
